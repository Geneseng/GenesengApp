#' Upload file with special JS
#' 
#' @param inputId Id of the input
#' @param label label of Btn
#' @param accept format of the input file
#' @param directory import folder
#'
#' @export
upload_biomarker_file <- function(inputId = "upload_file", label = "Upload File", directory = NULL){
  
  if(is.null(directory) | missing(directory)){
    directory <- NULL
  } else {
    directory <- TRUE
  }
